...
const allowed = new Set(["cv-tips","linkedin","career-growth","kenya-market"]);
const mapCategory = (c) => ({
...
const canonicalOf = (category, slug, current) => {
  if (!current || current.startsWith("/")) return `${SITE}/blog/${category}/${slug}`;
  try { new URL(current); return current; } catch { return `${SITE}/blog/${category}/${slug}`; }
};
...
const run = async () => {
  ...
  for (const fp of files) {
    ...
    raw = stripBOM(raw);
    if (!raw.trim().startsWith("---")) { console.warn("[WARN] No frontmatter:", fp); continue; }
    ...
    data.category = mapCategory(String(data.category || path.basename(path.dirname(fp))));
    if (!allowed.has(data.category)) { console.warn("[WARN] Bad category -> career-growth:", fp, data.category); data.category = "career-growth"; }
    ...
    if (typeof data.ogImage === "string" && data.ogImage.startsWith("/assets/images/blog/")) {
      const parts = data.ogImage.split("/");
      if (parts.length >= 6) { parts[5] = data.category; data.ogImage = parts.join("/"); }
    }

    const yaml = YAML.stringify(data, { aliasDuplicateObjects: false }).trim();
    const out = `---\n${yaml}\n---\n\n${body.trim()}\n`;
    if (out !== raw) { fs.writeFileSync(fp, out, "utf8"); changed++; }
  }
